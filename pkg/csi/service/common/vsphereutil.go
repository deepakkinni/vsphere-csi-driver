...
func CreateBlockVolumeUtil(ctx context.Context, clusterFlavor cnstypes.CnsClusterFlavor, manager *Manager,
	...
	if spec.ContentSourceSnapshotID != "" {
		...
		compatibleDatastoreFound := false
		...
		for _, dsInfo := range datastoreInfoList {
			if dsInfo.Info.Url == cnsVolume.DatastoreUrl {
				...
				compatibleDatastoreFound = true
				break
			}
		}

		if !compatibleDatastoreFound {
			log.Errorf("failed to find source volume datastore URL: %+v in datastore list: %+v",
				cnsVolume.DatastoreUrl, datastoreInfoList)
			return nil, csifault.CSIInternalFault, logger.LogNewErrorf(log,
				"failed to get the compatible datastore for create volume from snapshot %s",
