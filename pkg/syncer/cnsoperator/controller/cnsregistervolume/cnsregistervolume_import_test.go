...
// TestReconcileDeferFcd_DiskMetadataPropagated verifies that diskUUID, diskPath,
// vmName, and vmInstanceUUID are correctly propagated from the CRV spec to the
// CsiVolumeInfo spec, and that a URL-format DiskURLPath is normalized to the
// canonical [datastoreName] relPath format before being stored.
func TestReconcileDeferFcd_DiskMetadataPropagated(t *testing.T) {
	...
	// Override DiskURLPath with a real vCenter HTTP folder URL to validate that
	// normalizeDiskURLPath is applied before writing to CsiVolumeInfo.Spec.DiskPath.
	crv.Spec.DiskURLPath = "https://vc.example.com/folder/uuid/vm-test.vmdk" +
		"?dcPath=%2Fdc&dsName=sharedVmfs-0"
	stubIsPVCBound(t)
	r := newImportReconciler(t, pvc, crv, true)
	...
	// DiskPath must be normalized — not the raw URL.
	wantDiskPath := "[sharedVmfs-0] uuid/vm-test.vmdk"
	if cvi.Spec.DiskPath != wantDiskPath {
		t.Errorf("CVI.spec.diskPath = %q; want normalized %q", cvi.Spec.DiskPath, wantDiskPath)
	}
...
// TestNormalizeDiskURLPath verifies that normalizeDiskURLPath converts vCenter
// HTTP folder URLs to the canonical [datastoreName] relPath format and leaves
// already-normalized or unrecognised inputs unchanged.
func TestNormalizeDiskURLPath(t *testing.T) {
	tests := []struct {
		name  string
		input string
		want  string
		...
		{
			name: "https URL with dsName and dcPath",
			input: "https://vc.example.com:443/folder/uuid/vm-test-0652-3_4.vmdk" +
				"?dcPath=%2Ftest-dc&dsName=sharedVmfs-0",
			want: "[sharedVmfs-0] uuid/vm-test-0652-3_4.vmdk",
		},
		{
			name:  "https URL with nested folder path",
			input: "https://vc.example.com/folder/testns/imported-vm/disk-0.vmdk?dcPath=%2Fdc&dsName=vsanDatastore",
			want:  "[vsanDatastore] testns/imported-vm/disk-0.vmdk",
		},
		{
			name:  "http URL (non-TLS)",
			input: "http://vc.example.com/folder/uuid/disk.vmdk?dcPath=%2Fdc&dsName=myDS",
			want:  "[myDS] uuid/disk.vmdk",
		},
		{
			name:  "already normalized — returned unchanged",
			input: "[vsanDatastore] testns/imported-vm/disk-0.vmdk",
			want:  "[vsanDatastore] testns/imported-vm/disk-0.vmdk",
		},
		{
			name:  "URL missing dsName — returned unchanged",
			input: "https://vc.example.com/folder/uuid/disk.vmdk?dcPath=%2Fdc",
			want:  "https://vc.example.com/folder/uuid/disk.vmdk?dcPath=%2Fdc",
		},
		{
			name:  "URL with no /folder/ prefix — returned unchanged",
			input: "https://vc.example.com/datastore/uuid/disk.vmdk?dsName=myDS",
			want:  "https://vc.example.com/datastore/uuid/disk.vmdk?dsName=myDS",
		},
		{
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			got := normalizeDiskURLPath(tc.input)
			if got != tc.want {
				t.Errorf("normalizeDiskURLPath(%q) = %q; want %q", tc.input, got, tc.want)
			}
		})
	}
}
