...
func (r *ReconcileCnsRegisterVolume) ensureCsiVolumeInfoForImport(ctx context.Context,
	...
	cvi := &csivolumeinfov1alpha1.CsiVolumeInfo{
		...
		Spec: csivolumeinfov1alpha1.CsiVolumeInfoSpec{
			...
			DiskPath:     normalizeDiskURLPath(instance.Spec.DiskURLPath),
...
// normalizeDiskURLPath converts a vCenter HTTP folder URL to the canonical
// datastore-path format ([datastoreName] relPath) expected by CsiVolumeInfo.Spec.DiskPath.
// Input: https://host:443/folder/<relPath>?dcPath=...&dsName=<ds>
// Output: [<ds>] <relPath>
// If the input is already in datastore-path format or cannot be parsed, it is
// returned unchanged.
func normalizeDiskURLPath(diskURLPath string) string {
	if !strings.HasPrefix(diskURLPath, "https://") && !strings.HasPrefix(diskURLPath, "http://") {
		return diskURLPath
	}
	u, err := url.Parse(diskURLPath)
	if err != nil {
		return diskURLPath
	}
	dsName := u.Query().Get("dsName")
	relPath := strings.TrimPrefix(u.Path, "/folder/")
	if dsName == "" || relPath == u.Path {
		return diskURLPath
	}
	return fmt.Sprintf("[%s] %s", dsName, relPath)
}
